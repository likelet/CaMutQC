...
mutFilterPON <- function(maf, PONfile, PONformat = "vcf") {
    # give recommendation for PON file
    url <- "https://gatk.broadinstitute.org/hc/en-us/articles/360035890631-Panel-of-Normals-PON-"
    mes <- paste0("PON file can be obtained from Public GATK Panel of Normals at ",
                  url)
    message(mes)
    ...
    if (length(genomeVersion) > 1){
      ...
      if (PONformat == "vcf") {
          ...
          # give PON data a unique label, so they can be easily found and targeted
          somatic@fix[, 'ID'] <- 1
