...
#' @param clinicalData Clinical data associated with each sample/Tumor_Sample_Barcode in MAF. 
#' Could be a text file or a data.frame. Default NULL. Inherited from maftools.
...
#' If TRUE removes all the top 20 FLAG genes. If integer, remove top n FLAG genes.
#' Inherited from maftools.
...
#' order with gene name, Sample name and copy number status (either 'Amp' or 'Del'). 
...
#' Inherited from maftools.
#' @param removeDuplicatedVariants removes repeated variants in a particuar sample, 
#' mapped to multiple transcripts of same Gene. See Description. Default TRUE.
#' Inherited from maftools.
