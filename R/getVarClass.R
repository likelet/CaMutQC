getVarClass <- function(vcf_consequence, variantType, inframe) {
  if (vcf_consequence %in% c('splice_acceptor_variant', 
    ...
    return('Splice_Site')
  }else if (vcf_consequence == 'stop_gained') {
    return('Nonsense_Mutation')
  } else if (vcf_consequence == 'stop_lost') {
    return('Nonstop_Mutation')
  }else if(vcf_consequence == 'splice_region_variant') {
    return('Splice_Region')
  }else if(vcf_consequence == '3_prime_UTR_variant') {
    return('3\'UTR')
  }else if(vcf_consequence == 'upstream_gene_variant') {
    return('5\'Flank')
  }else if(vcf_consequence == 'downstream_gene_variant') {
    return('3\'Flank')
  }else if(vcf_consequence %in% c('transcript_amplification',
                                  'intron_variant', 'INTRAGENIC', 
                                  'intragenic_variant')) {
    return('Intron')
    ...
    return('Silent')
  }else if(vcf_consequence %in% c('mature_miRNA_variant', 'exon_variant', 
    ...
    return('5\'UTR')
  }else if(vcf_consequence %in% c('TF_binding_site_variant', 
                                  'regulatory_region_variant', 
                                  'regulatory_region', 'intergenic_variant', 
                                  'intergenic_region')) {
    return('IGR')
  }else if(vcf_consequence %in% c('missense_variant', 'coding_sequence_variant', 
