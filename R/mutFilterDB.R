...
mutFilterDB <- function(maf, dbVAF = 0.01, ExAC = TRUE, 
  ...
  if (Genomesprojects1000){
    if ('AF' %in% colnames(maf)){
      tags2 <- rownames(maf[((maf$AF != '') & (as.numeric(maf$AF) >= dbVAF)), ])
    }else{
      tags2 <- NULL
  ...
  if (gnomAD){
    if ('gnomAD_AF' %in% colnames(maf)){
      tags3 <- rownames(maf[((maf$gnomAD_AF != '') &
                               (as.numeric(maf$gnomAD_AF) >= dbVAF)), ])
    }else{
      tags3 <- NULL
  ...
  if (ESP6500){
    if ('AA_AF' %in% colnames(maf)){
      tags4 <- rownames(maf[((maf$AA_AF != '') &
                               (as.numeric(maf$AA_AF) >= dbVAF)), ])
    }else{
      tags4 <- NULL
      if ('EA_AF' %in% colnames(maf)){
        tags5 <- rownames(maf[((maf$EA_AF != '') &
                                 (as.numeric(maf$EA_AF) >= dbVAF)), ])
      }else{
        tags5 <- NULL
