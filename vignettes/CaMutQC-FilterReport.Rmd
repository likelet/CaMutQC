...
* Tumor sample name: <span style="color:#ED6F3E">**`r tumorSample`**</span>
* Normal Sample name: <span style="color:#ED6F3E">**`r unique(maf$Matched_Norm_Sample_Barcode)`**</span>
* Size of MAF data: <span style="color:#ED6F3E">**`r paste0(as.character(round(object.size(maf)/1024, 2)), ' kb')`**</span>
...
- tumor depth (tumorDP): <span style="color:#ED6F3E">**`r tumorDP`**</span>
- tumor allele depth (tumorAD): <span style="color:#ED6F3E">**`r tumorAD`**</span>
- normal depth (normalDP): <span style="color:#ED6F3E">**`r normalDP`**</span>
...
Some database published germline variants and recurrent artifacts in distinct races. In CaMutQC, based on the parameters we collected [3](#refer) [4](#refer) [5](#refer), potential germline variants will be removed using annotation from these database(if available) unless CliVar/OMIM/HGMD flags it as pathogenic.
...
dbs <- paste0(c('ExAC', 'Genomesprojects1000', 
                'ESP6500', 'gnomAD')[c(isTRUE(ExAC), isTRUE(Genomesprojects1000),
                                       isTRUE(ESP6500),isTRUE(gnomAD))], 
              collapse = ', ')
...
- **Only keep variants in COSMIC**: <span style="color:#ED6F3E">**`r COSMIConly`**</span>
...
- GRCh37: <span style="color:#ED6F3E">**somatic-b37_Mutect2-exome-panel.vcf**</span>
- GRCh38: <span style="color:#ED6F3E">**somatic-hg38_1000g_pon.hg38.vcf**</span>
...
Most studies relate to cancer somatic mutations remove certain types of variants in order to better target driven variants, among which **`exonic`** and **`nonsynonymous`** are two of the most widely used categories for filtration [4](#refer) [9](#refer) [10](#refer).
...
**Parameters in filtration and selection process refer to :** <span style="color:#ED6F3E">**`r if (exists('cancerType')){return(cancerType)}else{return('NULL')}`**</span>
...
In CaMutQC, three methods are supported for TMB calculation, including FoundationOne, MSK-IMPACT (3 versions of genelist) and Pan-cancer panel [16](#refer). By default, TMB is calculated using MSK-IMPACT method (gene panel version 3, 468 genes).
...
**Method used to calculate TMB**: <span style="color:#ED6F3E">**`r if(exists('assay')){return(assay)}else{return('Calculation not asked.')}`**</span>
